...
const int dispResX = 1280;
const int dispResY = 960;

//Projector: 1280 x 1024
//Monitor: 1280 x 960
int prevState = 1;
...
void loop() {
  ...
  if (bleMouse.isConnected()) {
    ...
    while (uartSerial.available()) {
      ...
      uint16_t lidar_x = (packet[2] << 8) | packet[3];
      uint16_t lidar_y = (packet[4] << 8) | packet[5];
      ...
      // Serial.print(lidar_x);
      // Serial.print(' ');
      // Serial.print(lidar_y);
      // Serial.println();
      ...
      // inside your packet‑processing block:
      if (motion_state == 2 && prevState != 2) {
        // we just entered “tap” mode:
        bleMouse.click();

        // pause long enough to avoid the next packet firing another click
        delay(100);  // adjust this (in ms) to taste

        // (don’t do any more moving/clicking until next packet)
